...
rule concatenate_fastq:
  ...
  output:
    ...
    total_reads_file = os.path.join(config['tmp_dir'], "samples", "{sample}", "{sample}_totalreads.csv")
  resources:
    mem_mb = 600
  conda:
    "../envs/gzip.yml"
  threads: 1
  message: "Decompressing reads if gzip'ed and concatenating into a single file, then calculate total number of reads"
  shell:
    """
